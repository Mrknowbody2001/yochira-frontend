...
export default function GetOneApprovedSo() {
  ...
  const grandTotal = items.reduce((sum, i) => sum + calculateRowValues(i).total, 0);
  const totalPriceDifference = items.reduce((sum, i) => sum + calculateRowValues(i).priceDifference, 0);
 ...
 const handleCreateSORN = async () => {
  ...
  const sornPayload = {
    ...
    remark: order.remark || "",
    sornCreatedDate: new Date().toISOString(), // live date
  ...
  try {
    ...
    navigate("/dashboard?tab=SORNList");
  } catch (err) {
    console.error(err);
    ...
    <div className="p-6 bg-gray-900 text-white rounded-lg">
      ...
      <Card className="bg-gray-800 mb-6">
        <div className="grid grid-cols-3 gap-4">
          ...
          <div>
            <label className="block text-sm font-medium text-gray-300">Create Date</label>
            <p className="mt-1 text-white">
          ...
          <div className="block">
            ...
            <p className="mt-1 text-white whitespace-pre-wrap">{order.paymentType || "-"}</p>
          </div>
        </div>
      </Card>
            ...
            <table className="w-full text-sm text-white border border-gray-700">
                ...
                {items.map((item, idx) => {
                  ...
                  return (
                    <tr key={idx} className="bg-gray-600">
                      ...
                      <td className="px-2 py-1 text-center">{(item.unitPrice ?? 0).toFixed(2)}</td>
                      <td className="px-2 py-1 text-center">{item.unitName}</td>
                      <td className="px-2 py-1 text-center">{total.toFixed(2)}</td>
                      ...
                      <td className="px-2 py-1 text-center">{priceDifference.toFixed(2)}</td>
                    </tr>
                  );
                })}
              </tbody>
              <tfoot>
                <tr className="bg-gray-700 font-semibold">
                  <td colSpan="7" className="text-center px-4 py-2">Grand Total</td>
                  <td className="px-2 py-1">{grandTotal.toFixed(2)}</td>
                  <td colSpan="2"></td>
                </tr>
                <tr className="bg-gray-700 font-semibold">
                  <td colSpan="7" className="text-center px-4 py-2">Total Price Difference</td>
                  <td className="px-2 py-1">{totalPriceDifference.toFixed(2)}</td>
                </tr>
              </tfoot>
            </table>
          </div>
      ...
      <div className="flex gap-4 mt-6">
        <Button color="green" onClick={handleCreateSORN}>Create SORN</Button>
        <Button color="gray" onClick={() => navigate("/dashboard?tab=ApprovedSOList")}>
          Cancel
        </Button>
      </div>
    </div>
