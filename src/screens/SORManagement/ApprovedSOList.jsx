...
const ApprovedSOList = () => {
  const [orderList, setOrderList] = useState([]);

  const fetchOrders = async () => {
    try {
      const res = await fetch(
        "http://localhost:5007/api/supplier-orders/approved-orders"
      );
      const data = await res.json();
      setOrderList(data || []);
      console.log("Fetched Approved Orders:", data);
    } catch (err) {
  ...
  return (
    <div className="bg-[#172e75] text-white min-h-screen w-full p-6 rounded overflow-auto">
      <div className="flex justify-between items-center mb-4">
        <h2 className="text-xl font-semibold">Supplier Approved Order List</h2>
      </div>
            ...
            <TableBody className="divide-y">
              {orderList.map((order, index) => (
                <TableRow key={order._id || index}>
                  ...
                  <TableCell>
                    <Link
                      to={`/dashboard?tab=GetOneApprovedSo&id=${order._id}`}
